...
- **layout**: AppShell (fixed rail slot + header slot + scrollable content — kills per-app Shell rebuilds), PageColumn (tokenised max-width + gutters, full-bleed opt-out — kills the negative-margin hack), Card (the dashboard hand-rolls a CARD literal today; clickable two ways — `as="a"`/`as="button"` when the card is the only target, or `interactive` + CardLink when it holds other controls)
...
55 exported components, counting `RailItem`, `ToastViewport` and `CardLink` as their own. `ChartTooltip` and `src/internal/` (`cx`, `month-grid`) are shared internals and stay off the barrel on purpose. Later waves: whatever the next dashboard actually needs — nothing speculative beyond this kit.
